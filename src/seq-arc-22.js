// Add check animation
if ($("#wf-form-Application-Form").length) {
  Webflow.push(function () {
    ...
    function buildPie() {
      const requiredFields = $("input[required], select[required]").splice(
        0,
        6
      );
      let count = 0;
    ...
    function saveUsr(el) {
      ...
      if (
        ...
        $(
          "#wf-form-Subform input[data-sub=" +
            el.currentTarget.dataset.sub +
            "]"
        ).val(
          $(
      ...
      if (name == "File 1") {
        if (!val.includes(".pdf")) {
          ...
          $(".success-state").hide();
          $(".error-state")
            .text("The company presentation must me a PDF")
            .show();
        }
      }
      ...
      if (el.currentTarget.type === "checkbox") {
        ...
        if (
          usrThemes.length >= 2 &&
          !usrThemes.includes(el.currentTarget.name)
        ) {
